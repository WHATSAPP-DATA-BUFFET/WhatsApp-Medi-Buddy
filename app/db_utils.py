...
# llm = ChatMistralAI(model="mistral-large-latest",api_key=uft_api_key)
...
def get_data(question):
    ...
    _DEFAULT_TEMPLATE = f"""You are an expert in writing sql queries.
        ...
        note: 
            -Do not give the explanation about the query,just give answer retrieved from the database alone in  final response.

            -Do not give any other extra informations along with the Answer ,just give the Answer alone.
    ...
    # db_chain = SQLDatabaseSequentialChain.from_llm(llm=llm,db=db)
...
def get_data_insurance(question):
    ...
    _DEFAULT_TEMPLATE = f"""You are an expert in writing sql queries.
        ...
        if the user question contains 'date of birth/2023-03-17':Responses should be fetched from the 'date_of_birth' column *case sensitive.
...
def get_data_lead(question):
    ...
    _DEFAULT_TEMPLATE = f"""You are an expert in writing sql queries.
        ...
        if the user question contains 'date of birth':Responses should be fetched from the 'date_of_birth' column *case sensitive.
